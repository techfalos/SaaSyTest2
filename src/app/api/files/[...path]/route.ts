...
function validateAndResolvePath(requestedPath: string[]): string | null {
  ...
  if (!resolvedFull.startsWith(resolvedBase)) {
    console.error(`🔒 Security: Attempted directory traversal blocked. Requested: ${fullPath}, Resolved: ${resolvedFull}, Base: ${resolvedBase}`);
    return null;
  }
  ...
  if (resolvedFull.startsWith(parentDir) && !resolvedFull.startsWith(resolvedBase)) {
    console.error(`🔒 Security: Attempted to access parent directory blocked. Path: ${resolvedFull}`);
    return null;
  }
