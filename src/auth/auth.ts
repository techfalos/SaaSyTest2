...
export async function check_auth(db:any, db_select:any): Promise<{UserID:string, UserLevel:number, IsAdmin:boolean}> {
  ...
  if (session.length > 0) {
    ...
    const user = await db_select(db, 'User', ['UserLevel', 'IsAdmin'], [session[0].UserID], undefined);
    if (user.length > 0) {
      sessionInfo.UserLevel = tiers.findIndex(tier => tier.name === user[0].UserLevel);
      sessionInfo.IsAdmin = user[0].IsAdmin;
    }
  }
  return sessionInfo;
}   